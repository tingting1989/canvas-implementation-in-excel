...
```
Phase 1（1-2 周）— 核心可用，不上线会出事故
├── P1-1  ShadowEvaluator 沙箱隔离        ← 致命风险（唯一不可回滚点）
├── P1-2  UI 控制器（下拉菜单+错误提示）   ← 用户不可见等于没有
└── P1-3  ListValidator 动态区域引用       ← 高频使用场景

Phase 2（1-2 周）— 性能与完整性
├── P2-1  脏标记机制                      ← 10K 行性能关键
├── P2-2  Sort/Paste/AutoFill 集成        ← 批量操作不卡顿
├── P2-3  复制/粘贴规则行为               ← 与 Excel 行为一致
└── P2-4  条件格式 errorStyle 差异化       ← 视觉反馈正确性

/** 按值筛选配置 */
export interface FilterConfigValues {
    type: "values";
    uncheckedValues: Set<string>;
}

/** 按条件筛选配置 */
export interface FilterConfigCondition {
    type: "condition";
    operator: string;
    value: string;
    valueEnd?: string;
}

/** 筛选配置联合类型 */
export type FilterConfig = FilterConfigValues | FilterConfigCondition;

/**
 * 筛选状态管理器 (Filter State Manager)
 *
 * 负责管理所有列的筛选状态，包括：
 * - 各列的筛选配置（勾选项/条件）
 * - 唯一值缓存（避免重复计算）
 * - 缓存失效标记（数据变化时标记需要刷新）
 *
 * ## 数据结构
 * ```
 * #columnFilters: Map<colIndex, FilterConfig>
 *     FilterConfig = { type: "values", uncheckedValues: Set<string> }
 *                   | { type: "condition", operator: string, value: string }
 *
 * #uniqueValuesCache: Map<colIndex, string[]>  - 缓存各列唯一值
 * #invalidatedColumns: Set<colIndex>            - 标记需要重新计算唯一值的列
 * ```
 *
 * ## 缓存策略
 * - 打开筛选面板时：读取缓存 → 有效则直接使用，无效则重新计算并缓存
 * - 数据变化时：标记相关列为失效 → 下次打开时重新计算
 *
 * @example
 * const state = new FilterState();
 * state.setColumnFilter(0, { type: "values", uncheckedValues: new Set(["apple"]) });
 * state.getColumnFilter(0);  // { type: "values", uncheckedValues: Set(1) }
 *
 * @module plugins/filter/FilterState
 */
export class FilterState {
    /** @private 私有字段 - 列索引 → 筛选配置 */
    #columnFilters: Map<number, FilterConfig> = new Map();

    /** @private 私有字段 - 列索引 → 缓存的唯一值列表 */
    #uniqueValuesCache: Map<number, string[]> = new Map();

    /** @private 私有字段 - 已失效的列索引集合（需要重新计算唯一值） */
    #invalidatedColumns: Set<number> = new Set();

    /**
     * 设置列的筛选配置
     *
     * @param col - 列索引
     * @param filter - 筛选配置对象
     */
    setColumnFilter(col: number, filter: FilterConfig): void {
        this.#columnFilters.set(col, filter);
    }

    /**
     * 移除列的筛选配置
     *
     * 同时清除该列唯一值缓存。
     *
     * @param col - 列索引
     */
    removeColumnFilter(col: number): void {
        this.#columnFilters.delete(col);
        this.#uniqueValuesCache.delete(col);
    }

    /**
     * 获取列的筛选配置
     *
     * @param col - 列索引
     * @returns 筛选配置，不存在则返回 null
     */
    getColumnFilter(col: number): FilterConfig | null {
        return this.#columnFilters.get(col) || null;
    }

    /**
     * 获取所有列的筛选配置副本
     *
     * @returns 所有筛选配置的浅拷贝
     */
    getAllFilters(): Map<number, FilterConfig> {
        return new Map(this.#columnFilters);
    }

    /**
     * 是否存在任何激活的筛选
     *
     * @returns 至少有一个列配置了筛选时返回 true
     */
    hasActiveFilters(): boolean {
        return this.#columnFilters.size > 0;
    }

    /**
     * 清除所有筛选状态
     *
     * 包括：所有列筛选配置、唯一值缓存、失效标记
     */
    clearAll(): void {
        this.#columnFilters.clear();
        this.#uniqueValuesCache.clear();
        this.#invalidatedColumns.clear();
    }

    /**
     * 缓存列的唯一值列表
     *
     * @param col - 列索引
     * @param values - 该列的所有唯一值
     */
    cacheUniqueValues(col: number, values: string[]): void {
        this.#uniqueValuesCache.set(col, values);
    }

    /**
     * 获取列缓存的唯一值
     *
     * @param col - 列索引
     * @returns 缓存的唯一值列表，不存在则返回 null
     */
    getUniqueValuesCache(col: number): string[] | null {
        return this.#uniqueValuesCache.get(col) || null;
    }

    /**
     * 标记列的缓存为失效
     *
     * 当单元格数据变化时调用，之后该列重新打开筛选面板时会重新计算唯一值。
     *
     * @param col - 列索引，undefined 表示所有列
     */
    invalidateColumnCache(col?: number): void {
        if (col !== undefined) {
            this.#invalidatedColumns.add(col);
            this.#uniqueValuesCache.delete(col);
        } else {
            this.#uniqueValuesCache.clear();
        }
    }

    /**
     * 检查列的唯一值缓存是否有效
     *
     * @param col - 列索引
     * @returns 缓存有效（未失效）返回 true
     */
    isCacheValid(col: number): boolean {
        return !this.#invalidatedColumns.has(col);
    }
}

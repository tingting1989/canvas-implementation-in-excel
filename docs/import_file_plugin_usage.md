...
try {
    const result = await importPlugin.importFromFile(file, {
        startRow: 0,           // 起始行（默认 0）
        startCol: 0,
        // 是否将第一行作为表头，当 嵌套表头时用：false// 起始列（默认 0）
        ...
        applyMerges: true,      // 应用合并单元格（默认 true）
        applyDimensions: true,  // 应用列宽和行高（默认 true）
        headerRows: 1,        // 表头行数（用于嵌套表头场景），2 (2层嵌套表头)
        dataStartRow: 2,      // 数据起始行（可选，默认从 startRow 开始），2 (跳过2行表头)
    });
...
| `applyMerges` | boolean | true | 是否应用合并单元格 |
| `applyDimensions` | boolean | true | 是否应用列宽和行高 |
| `headerRows` | number | 1 | 表头行数（用于嵌套表头场景），2 (2层嵌套表头) |
| `dataStartRow` | number | - | 数据起始行（可选，默认从 startRow 开始），2 (跳过2行表头) |

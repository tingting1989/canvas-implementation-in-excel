...
export const mathFunctions = {
    ...
    AVERAGE: (args) => {
        ...
        if (flat.length === 0) {
            errorHandler.warn(ERROR_CODE.FORMULA_DIV_ZERO, "AVERAGE: 没有有效的数值可计算", { functionName: "AVERAGE" });
            return "#DIV/0!";
        }
    ...
    ROUND: (args) => {
        ...
        if (isNaN(digits) || digits < 0) {
            errorHandler.warn(ERROR_CODE.FORMULA_EVAL_ERROR, "ROUND: 第二个参数必须是非负整数", { value: args[1], functionName: "ROUND" });
            return "#VALUE!";
        }

 ...
 * @param maxArgs - 最大参数数量（不限制时传 Infinity）
 * @param functionName - 函数名称（用于日志）
 * @returns 校验是否通过（true=通过，false=失败）
 *
 * @example
 * // 基础用法：SUM 至少需要 1 个参数
 ...
 */
export function _validateArgs(args: unknown[], minArgs: number, maxArgs: number = Infinity, functionName: string): boolean {
 ...
 */
export function _validateNumericArg(args: unknown[], index: number, paramName: string, functionName: string): { valid: boolean; value?: number } {

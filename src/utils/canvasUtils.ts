 ...
 */
export function extractFontSize(font: string, fallback?: number): number {
 ...
 * ```
 *
 * @param areaY - 区域顶部 Y 坐标
 * @param areaHeight - 区域高度（px）
 * @param fontOrSize - CSS font 字符串 或 字体大小数字（px）
 * @returns 垂直居中的 textY 坐标（基线位置）
 *
 * @example
 * // 用于表头文字居中
 ...
 * // 返回: 100 + 15 + 4.2 = 119.2
 */
export function calcCenteredTextY(areaY: number, areaHeight: number, fontOrSize: string | number): number {
 ...
 * 计算水平居中的 X 坐标
 *
 * @param areaX - 区域左侧 X 坐标
 * @param areaWidth - 区域宽度（px）
 * @returns 水平居中的 X 坐标
 *
 * @example
 * calcCenteredTextX(0, 100);  // 50
 */
export function calcCenteredTextX(areaX: number, areaWidth: number): number {
 ...
 */
export function getAreaCenter(x: number, y: number, width: number, height: number): { centerX: number; centerY: number } {
    return {
        centerX: x + width / 2,
        centerY: y + height / 2,
    };

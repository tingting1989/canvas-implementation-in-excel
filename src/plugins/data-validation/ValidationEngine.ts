...
type ConflictStrategy = "short-circuit" | "priority" | "aggregate";
...
export class ValidationEngine {
    ...
    #conflictStrategy: ConflictStrategy = "short-circuit";
    ...
    init(formulaEngine: any = null, sheetManager: any = null, config: EngineConfig = {}): void {
        ...
        const formulaConfig = {
            maxComplexity: 100,
            timeout: config.asyncTimeout || 500,
            enableShadowEval: config.enableDeferred !== false,
        };
                ...
                try {
                    ...
                    if (cached && cached.result !== null && cached.result !== undefined) {
                        return cached.result.valid
                            ? ValidationResult.success({ source: "advanced-cache", level: cached.source })
    ...
    validateCellSync(row: number, col: number, value: any): ValidationResult {
        ...
        const context: ValidateContext = { row, col, sheet: this.#cellStore?.sheetName || "" };
    ...
    async validateRange(range: string): Promise<RangeValidateReport> {
    ...
    async validateWithShortCircuit(rules: ValidationRule[], value: any, context: ValidateContext): Promise<ValidationResult> {
    ...
    async validateWithPriority(rules: ValidationRule[], value: any, context: ValidateContext): Promise<ValidationResult> {
    ...
    async validateWithAggregate(rules: ValidationRule[], value: any, context: ValidateContext): Promise<ValidationResult> {
        const errors: string[] = [];
        for (const rule of rules) {
            const validator = this.#validators.get(rule.type);
            if (!validator) continue;
    ...
    validateWithShortCircuitSync(rules: ValidationRule[], value: any, context: ValidateContext): ValidationResult {
        for (const rule of rules) {
            const validator = this.#validators.get(rule.type);
            if (!validator) continue;
    ...
    validateWithPrioritySync(rules: ValidationRule[], value: any, context: ValidateContext): ValidationResult {
        let lastResult: ValidationResult = ValidationResult.success();
        for (const rule of rules) {
            const validator = this.#validators.get(rule.type);
            if (!validator) continue;
    ...
    validateWithAggregateSync(rules: ValidationRule[], value: any, context: ValidateContext): ValidationResult {
        const errors: string[] = [];
        for (const rule of rules) {
            const validator = this.#validators.get(rule.type);
            if (!validator) continue;
    ...
    getCellsInRange(rangeStr: string): { row: number; col: number; value: any }[] {
    ...
    setToCache(key: string, value: any, result: ValidationResult): void {
        ...
        if (cellCache.size >= this.#maxCacheSize) {
            ...
            if (oldestKey !== undefined) cellCache.delete(oldestKey);
        }
    ...
    set conflictStrategy(strategy: ConflictStrategy) {
        const validStrategies: ConflictStrategy[] = ["short-circuit", "priority", "aggregate"];
    ...
    get conflictStrategy(): ConflictStrategy {

...
|------|---------|------------------|------|
| [NumberValidator.js](../src/plugins/data-validation/validators/NumberValidator.js) | 数值范围 (`number`) | between, greaterThan, lessThan, equalTo 等 (8个) | ✅ Phase 1 |
| [TextLengthValidator.js](../src/plugins/data-validation/validators/TextLengthValidator.js) | 文本长度 (`text`) | lengthBetween, lengthGreaterThan 等 (8个) | ✅ Phase 1 |
...
| **Web Worker 异步验证** | 超大数据集批量验证移至 Worker 线程 | ❌ 尚未引入 Worker，BatchValidationCoordinator 在主线程运行 |
...
当**同一个单元格**被多条验证规则覆盖时，引擎需要决定如何处理验证结果。通过 `conflictStrategy` 配置解决策略：
...
async validateWithPriority(rules, value, context) {
    ...
    for (const rule of rules) {
        const validator = this.#validators.get(rule.type);
        lastResult = await validator.validate(value, rule, context);  // ← 全部执行完
    }
...
// 规则 B：自定义公式校验 — 必须是5的倍数（priority=1）
dv.setValidation({
    range: 'B1:B10',
    type: 'custom',
    formula: '=MOD(B2,5)=0',
    errorMessage: '必须是 5 的倍数',
    priority: 1,
});
...
用户在 B2 输入值 `150` 时，各策略的行为差异：

| 策略 | 执行过程 | 返回的 message | errorStyle |
|------|---------|---------------|------------|
| **`short-circuit`** | A(失败) → **立即返回**，B、C 不再执行 | `"必须在 0-100 之间"` | `stop`（来自规则A） |
| **`priority`** | A(失败) → B(失败) → C(通过)，全部执行完 | `"不能重复"` | `warning`（来自最后执行的规则C） |
| **`aggregate`** | A(失败) → B(失败) → C(通过)，收集所有错误 | `"必须在 5 的倍数；不能重复"` | `warning`（分号拼接） |
...
| ✅ **ValidationPortal UI** | **已实现** | [ValidationPortalManager.js](../src/plugins/data-validation/ValidationPortalManager.js) — 基于 DOMComponent 的 Portal 渲染（下拉菜单、错误提示、气泡框） |
...
dv.setValidation({
    ...
    errorMessage: '必须是 2024-2025 年的工作日（周一至周五）'
});
...
// 示例 7: 数值精度 — 必须是 0.01 的整数倍（两位小数金额）
dv.setValidation({
    range: 'G1:G200',
    type: 'custom',
...
dv.setValidation({
    ...
    type: 'filename',
    maxLength: 100,
    errorMessage: '文件名包含非法字符或不满足命名规范',
    errorStyle: 'stop'
});
```
...
abstract class BaseValidator {
    /**
     * 子类必须实现的静态属性 - 类型标识符
     * 用于 ValidationEngine 按 type 分发到对应验证器
     */
...
| **利用 `rule` 对象传递参数** | 自定义字段放在 `rule.value` 或直接挂在 rule 上 |

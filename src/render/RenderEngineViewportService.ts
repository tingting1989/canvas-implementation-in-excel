...
export class RenderEngineViewportService extends ViewportService {
    ...
    get scrollX(): any {
        return (this.#renderEngine as any).scrollX;
    }

    /** 当前垂直滚动偏移（数据坐标） @override */
    get scrollY(): any {
        return (this.#renderEngine as any).scrollY;
    }

    /** 视口宽度（CSS 像素） @override */
    get viewW(): any {
        return (this.#renderEngine as any).viewW;
    }

    /** 视口高度（CSS 像素） @override */
    get viewH(): any {
        return (this.#renderEngine as any).viewH;
    }

    /** 最大水平滚动偏移 @override */
    get maxScrollX(): any {
        return (this.#renderEngine as any).maxScrollX;
    }

    /** 最大垂直滚动偏移 @override */
    get maxScrollY(): any {
        return (this.#renderEngine as any).maxScrollY;
    }
     ...
     * @param row - 行索引
     * @param col - 列索引
     * @param mergeInfo - 合并单元格信息，有值时返回整个合并区域的矩形
     * @returns 视口坐标矩形
     * @override
     */
    getCellRect(row: number, col: number, mergeInfo: Record<string, unknown> | null = null): ViewRect {
        return (this.#renderEngine as any).getCellRect(row, col, mergeInfo);
    }
    ...
    hitTest(clientX: number, clientY: number): HitResult | null {
        return (this.#renderEngine as any).hitTest(clientX, clientY);
    }
     ...
     * @param clientX - 客户端 X 坐标
     * @param clientY - 客户端 Y 坐标
     * @returns 命中结果，如 { type: HIT_TYPE.COL_RESIZE, index }
     * @override
     */
    headerHitTest(clientX: number, clientY: number): HitResult | null {
        return (this.#renderEngine as any).headerHitTest(clientX, clientY);
    }
    ...
    fillHandleHitTest(clientX: number, clientY: number): boolean {
        return (this.#renderEngine as any).fillHandleHitTest(clientX, clientY);
    }
    ...
    scrollToCell(row: number, col: number): void {
        (this.#renderEngine as any).scrollToCell(row, col);
    }
    ...
    isCellVisible(row: number, col: number, canvasW: number, canvasH: number, tabH: number = 0): boolean {
        const sheet: Sheet | null = (this.#renderEngine as any).currentSheet;
        if (!sheet) return false;
        const vt = new ViewportTransform(sheet, (this.#renderEngine as any).scrollX, (this.#renderEngine as any).scrollY);
    ...
    setResizeLine(type: string, index: number, position: number): void {
        (this.#renderEngine as any).setResizeLine(type, index, position);
    }
    ...
    clearResizeLine(): void {
        (this.#renderEngine as any).clearResizeLine();
    }
     ...
     */
    invalidateAll(): void {
        (this.#renderEngine as any).invalidateAll();
    }
     ...
     */
    get chartLayer(): any {
        return (this.#renderEngine as any).chartLayer;
    }
}
